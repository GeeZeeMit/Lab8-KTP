...
public class CrawlerTask implements Runnable
{
    ...
    {
        while (true) { Process(m_Pool.get()); }
    }
    private void Process(URLDepth pair) throws IOException
    {
        ...
        while ((input = reader.readLine()) != null)
        {
            while (input.contains("a href=\"" + m_Prefix))
            {
                ...
                if (m_Pool.getNotProcessed().contains(new URLDepth(link, AnyDepth)) ||
                        m_Pool.getProcessed().contains(new URLDepth(link, AnyDepth))) continue;
                m_Pool.addNotProcessed(new URLDepth(link, pair.getDepth() - 1));
            }
        }
        reader.close();
